...
function KpiCard({
  label, value, delta, invert = false, emphasis = false,
}: {
  label: string; value: string; delta: number | null; invert?: boolean; emphasis?: boolean;
}) {
  ...
  return (
    <div className={`perf-kpi${emphasis ? ' is-emphasis' : ''}`}>
...
export function KpiGrid({ curr, prev }: Props) {
  ...
  return (
    <div className="perf-kpi-grid">
      <KpiCard label="Faturamento" value={fmtBRL(curr.revenue_cents)} delta={d(curr.revenue_cents, prev?.revenue_cents)} />
      <KpiCard label="Investido" value={fmtBRL(curr.spend_cents)} delta={d(curr.spend_cents, prev?.spend_cents)} invert />
      <KpiCard label="ROAS" value={fmtRoas(curr.roas)} delta={d(curr.roas, prev?.roas)} emphasis />
