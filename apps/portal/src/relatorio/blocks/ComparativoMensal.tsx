...
export function ComparativoMensal({ slug, margemCliente, monthsBack = 6 }: Props) {
  ...
  if (loading) return <div className="rep-block-loading">Comparando últimos {monthsBack} meses...</div>;
  if (meses.every((m) => m.revenue === 0)) return null;

  return (
    <div className="rep-block rep-comp-mensal">
      <div className="rep-block-head">
        <div>
          <div className="rep-section-kicker">Histórico mensal</div>
          <div className="rep-section-title">{monthsBack} meses lado a lado</div>
        </div>
      </div>
      <div className="rep-comp-table">
        <div className="rep-comp-row rep-comp-row--head">
          <div>Mês</div>
        ...
        {meses.map((m, i) => {
          ...
          return (
            <div key={m.label} className="rep-comp-row">
              <div className="rep-comp-mes">{m.label}</div>
              ...
              <div className={`ta-r mono rep-comp-delta--${tone}`}>{deltaPct == null ? '—' : `${arrow} ${Math.abs(deltaPct).toFixed(0)}%`}</div>
            </div>
          );
        })}
      </div>
