...
export function Tendencia({ slug, preset, since, until, margemCliente }: Props) {
  ...
  if (loading) return <div className="rep-block-loading">Carregando tendência...</div>;
  if (err) return <div className="rep-empty">Tendência indisponível: {err}</div>;
  ...
  return (
    <div className="rep-block rep-tendencia">
      <div className="rep-block-head">
        <div>
          <div className="rep-section-kicker">Tendência</div>
          <div className="rep-section-title">Histórico dia a dia</div>
        </div>
        <div className="rep-metric-tabs">
          {METRICS.map((m) => (
            <button
              key={m.key}
              type="button"
              className={`rep-metric-tab${m.key === metric ? ' is-on' : ''}`}
      ...
      <svg viewBox={`0 0 ${W} ${H}`} className="rep-tendencia-svg" preserveAspectRatio="none">
        {/* grid Y */}
      ...
      <div className="rep-tendencia-foot">
