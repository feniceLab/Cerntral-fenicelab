...
export function Breakdowns({ slug, preset, since, until }: Props) {
  ...
  return (
    <div className="rep-block rep-breakdowns">
      <div className="rep-block-head">
        <div>
          <div className="rep-section-kicker">Breakdowns</div>
          <div className="rep-section-title">De onde vem cada real</div>
        </div>
        <div className="rep-metric-tabs">
          {TABS.map((t) => (
            <button
              key={t.key}
              type="button"
              className={`rep-metric-tab${t.key === activeTab ? ' is-on' : ''}`}
      ...
      {loading && <div className="rep-block-loading">Carregando...</div>}
      {err && <div className="rep-empty">Indisponível: {err}</div>}
      {!loading && !err && rows.length === 0 && (
        <div className="rep-empty">Sem dados de {currentTab.label.toLowerCase()} no período.</div>
      )}

      {rows.length > 0 && (
        <div className="rep-breakdown-table">
          <div className="rep-bd-row rep-bd-row--head">
          ...
          {rows.map((row, i) => {
            ...
            return (
              <div key={i} className="rep-bd-row">
                <div className="rep-bd-dim">{dim}</div>
                <div className="ta-r mono">{fmtBRL(row.spend_cents)}</div>
                <div className="rep-bd-bar">
                  <div className="rep-bd-bar-fill" style={{ width: `${Math.max(2, pct)}%` }} />
                </div>
